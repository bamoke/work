  ...
  <div class="content-wrap">
    <PageCard :title="title" class="m-monitor-wrap">
      <template v-slot:extra>
        <van-button size="mini" style="width:80px;" type="info" @click="goBack()"
            ...
            <div class="danymic-wrap" v-show="showDynamic">
                  ...
                  <div class="filter-item">
                    <div class="filter-type section">
                      <select v-model="filterType">
                        <option value="zjz">增加值</option>
                        <option value="zcz">总产值</option>
                      </select>
                    </div>
                    <div class="params">
                      <van-radio-group
              ...
              >
                <div
                  ...
                  :key="index"
                  @click="handleSelectCate('curCate', item.text)"
                >
                  {{ item.text }}
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    getData(queryData = this.queryData) {
      ...
      comMonitorApi
        .get_pillar_com({ params: queryData, type: "zjz" })
        .then((res) => {
          this.title = res.data.title;
          this.grossName = res.data.grossName;
          this.pageInfo = res.data.pageInfo;
  ...
  mounted() {
    ...
    comMonitorApi.get_pillar_cate({ params: { type: "zjz" } }).then((res) => {
