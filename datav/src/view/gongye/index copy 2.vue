 ...
 * @LastEditTime: 2021-06-02 05:28:51
  ...
  <div class="content-wrap">
    ...
    <div class="row-big">
      ...
      <div class="row-big-child">
        ...
        <ModuleCard title="指标对比" class="item-wrap">
          <template v-slot:extra>
            <ButtonGroup shape="circle" size="small">
              <Button
                :type="echartChangeKey.zbdb == 'city' ? 'primary' : ''"
                @click="handleChangeZbdb('city')"
                >本市</Button
              >
              <Button
                :type="echartChangeKey.zbdb == 'country' ? 'primary' : ''"
                @click="handleChangeZbdb('country')"
                >国内</Button
              >
            </ButtonGroup>
          </template>
          <div id="zbdb" style="width: 100%; height: 260px"></div>
        </ModuleCard>
      </div>
      ...
      <div class="row-big-child">
        <ModuleCard
          ...
          class="item-wrap"
        >
          <div id="nyxf" style="width: 100%; height: 220px"></div>
        </ModuleCard>
        ...
        >
          <template v-slot:extra>
            <ButtonGroup shape="circle" size="small">
              <Button
                :type="echartChangeKey.nyxf == 'swl' ? 'primary' : ''"
                @click="handleChangeNyxf('swl')"
                >实物量</Button
              >
              <Button
                :type="echartChangeKey.nyxf == 'bzl' ? 'primary' : ''"
                @click="handleChangeNyxf('bzl')"
                >标准量</Button
              >
            </ButtonGroup>
          </template>
      ...
      <div class="row-big-child">
        <ModuleCard
          ...
          class="item-wrap"
        >
          <div id="ydlqk" style="width: 100%; height: 204px"></div>
        </ModuleCard>
        <ModuleCard title="工业企业用电监测(2021年3月)" class="item-wrap">
          <div style="width: 100%">
            <Table
              size="small"
              height="260"
    ...
    <div class="row-side">
      <div class="transform-box">
        ...
        <ModuleCard title="规模以上工业分行业工业总产值" class="item-wrap">
          <Table
            size="small"
            height="310"
            stripe
    ...
    <div class="side-row" style="display: none">
      <ModuleCard title="规模以上工业总产值" class="item-wrap">
        <div id="cz-gs" style="width: 100%; height: 320px"></div>
      </ModuleCard>

      <ModuleCard
        title="重点支柱产业工业总产值"
        :total-info="czZzcyData.total"
        class="item-wrap"
      >
        <div id="cz-zzcy" style="width: 100%; height: 320px"></div>
      </ModuleCard>

      <ModuleCard
        title="现代产业和规上民营工业企业总产值"
        :total-info="czXdcyData.total"
        class="item-wrap"
      >
        <div id="cz-xdcy" style="width: 100%; height: 320px"></div>
      </ModuleCard>

      <ModuleCard title="规模以上工业分行业工业总产值" class="item-wrap">
        <div id="cz-hy" style="width: 100%; height: 100%">
          <Table
            size="small"
            height="310"
            stripe
...
export default {
  ...
  methods: {
    ...
    chartInit({ themeName = this.$config.chartTheme } = {}) {
      var echartInstance = {};
      /**工业增加值图表初始化 */
      echartInstance.zjzGsChart = this.$echarts.init(
        document.getElementById("zjz-gs"),
        themeName
      );
      echartInstance.zjzZzcyChart = this.$echarts.init(
        document.getElementById("zjz-zzcy"),
        themeName
      );
      echartInstance.zjzXdcyChart = this.$echarts.init(
        document.getElementById("zjz-xdcy"),
        themeName
      );

      /**工业生产总值图表初始化 */
      echartInstance.czGsChart = this.$echarts.init(
        document.getElementById("cz-gs"),
        themeName
      );
      echartInstance.czZzcyChart = this.$echarts.init(
        document.getElementById("cz-zzcy"),
        themeName
      );
      echartInstance.czXdcyChart = this.$echarts.init(
        document.getElementById("cz-xdcy"),
        themeName
      );

      echartInstance.zbdbChart = this.$echarts.init(
        document.getElementById("zbdb"),
        themeName
      );

      //用电情况
      echartInstance.ydlqkChart = this.$echarts.init(
        document.getElementById("ydlqk"),
        themeName
      );
      echartInstance.nyxfChart = this.$echarts.init(
        document.getElementById("nyxf"),
        themeName
      );
  ...
  mounted() {
    ...
    /**增加值图表实例配置 */
    this.echartInstance.zjzGsChart.setOption({
      title: {
        text: "近12个月累计增长速度",
        top: "top",
      },
    ...
    this.echartInstance.zjzZzcyChart.setOption({
      title: {
        text: "增加值及增长率",
      },
      tooltip: {},
    ...
    this.echartInstance.zjzXdcyChart.setOption({
      legend: {
        data: ["金湾区", "金湾直属", "开发区"],
      },
      tooltip: {},
    ...
    /**生产总值图表实例配置 */
    this.echartInstance.czGsChart.setOption({
      title: {
        text: "累计增长速度",
        top: "top",
        ...chartTitleStyle,
    ...
    this.echartInstance.czZzcyChart.setOption({
      title: {
        text: "总产值及增长率",
      },
      tooltip: {},
    ...
    this.echartInstance.czXdcyChart.setOption({
      legend: {
        data: ["金湾区", "金湾直属", "开发区"],
      },
      tooltip: {},
    ...
    /**指标对比实例配置 */
    this.echartInstance.zbdbChart.setOption({
      title: {
        text: "各区规模以上工业增加值对比",
        top: "top",
      },
