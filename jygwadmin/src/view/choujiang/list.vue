...
export default {
  ...
  data() {
    return {
      columns: [
        { title: "抽奖标题", key: "title", width: 240 },
            ...
            return h(
              "Button",
              {
                props: {
                  size: "default",
                  icon: "ios-people",
                  to: { path: `${config.webBaseUrl}choujiang/awarded`,query:{cid:curItem.id} }
                }
              },
              curItem.join_num
            );
  ...
  methods: {
    ...
    handleAdd() {
      this.$router.push({ name: "choujiang_add" });
    },
    handleEdit(params) {
      const id = params.row.id;
      this.$router.push({ name: "choujiang_edit", params: { id } });
    },
    handleDelete(params) {
