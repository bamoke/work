...
export default {
  ...
  data() {
    return {
      columns: [
        { title: "项目标题", key: "title", width: 240 },
...

        {
          ...
          render: (h, params) => {
            ...
            let className = "";
            let stageTxt = "";
            let curStage = parseInt(params.row.stage);
            if (curStage === 0) {
            ...
            return h("span", { class: className }, stageTxt);
          },
          width: 120
        },
            ...
            return h(
              "Button",
              {
                props: {
                  size: "default",
                  icon: "ios-people",
                  to: { path: `${config.webBaseUrl}grants/apply`,query:{grantid:curItem.id} }
                }
              },
              curItem.apply_num
            );
  ...
  methods: {
    ...
    handleAdd() {
      this.$router.push({ name: "grants_add" });
    },
    handleEdit(params) {
      const id = params.row.id;
      this.$router.push({ name: "grants_edit", params: { id } });
    },
    handleDelete(params) {
