 ...
 * 加入params参数以适应动态路由//by joy on 2018-08-17
 */
export const getNewTagList = (list, newRoute) => {
  const { name, path, meta, params } = newRoute
  let newList = [...list]
  if (newList.findIndex(item => item.name === name) >= 0) return newList
  else newList.push({ name, path, meta, params })
  return newList
}
...
export const getNextRoute = (list, name) => {
  ...
  if (list.length === 2) {
    ...
    if (list.findIndex(item => item.name === name) === list.length - 1) {
      res.name = list[list.length - 2].name
      res.params = list[list.length - 2].params
    } else {
      res.name = list[list.findIndex(item => item.name === name) + 1].name
      res.params = list[list.findIndex(item => item.name === name) + 1].params
    }
  }
  return res
}
...
export const closeCurPage = function (vm) {
