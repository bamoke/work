  ...
  <div>
    <Card>
          ...
          <Row :gutter="8">
              <i-col span="3">
                <Select v-model="searchKey">
                  <Option v-for="item in searchKeyItem" :value="item.key" :key="item.key">{{ item.name }}</Option>
                </Select>
                </i-col>
                <i-col span="8">
      ...
      <div class="m-paging-wrap">
        <Page :total="pageInfo.total" :current="curPage" :page-size="pageInfo.pageSize" @on-change="changePage"></Page>
      </div>
    </Card>
    <Modal ref="userform-modal" v-model="showForm" draggable scrollable >
        ...
        <UserForm :uid="editId" @handle-submit="handleFormSubmit" @handle-cancel="handleFormCancel" />
        <p slot="footer"></p>
    </Modal>
  </div>
</template>
...
import { getUserlist, deleteUser } from "@/api/admin_user";
export default {
  name: "user-list",
  components: {
    UserForm
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { title: "昵称", key: "nickname", width: 150, sortable: true },
        { title: "角色", key: "role_name", width: 250 },
        {
          title: "状态",
          key: "status",
          width: 150,
  ...
  computed: {
    // editId: function(){
    //   return this.editRowIndex ? this.tableData[this.editRowIndex].id: 0
    // }
  },
  methods: {
    changeSearchKey(e) {
