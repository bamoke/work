 ...
 * @LastEditTime: 2021-02-28 18:15:18
...
<template>
  <div>
    <Card>
      ...
      <Button type="primary" icon="ios-add-circle" slot="extra" @click.prevent="handleAdd">新增员工</Button>
    ...
    <Modal ref="com-form-modal" v-model="showModal" draggable scrollable>
      <p slot="header" style="text-align:left">
        ...
        <span>{{formActype=='add'?'新增员工':'修改员工'}}</span>
      </p>
      <StationerForm :id="editId" @handle-submit="handleFormSubmit" @handle-cancel="handleFormCancel" />
      <p slot="footer"></p>
    </Modal>
  </div>
</template>
...
<script>
import StationerForm from "./main-form.vue";
import { getStationerList, deleteStationerOne } from "@/api/stationer";
export default {
  name: "stationer-list",
  components:{StationerForm},
  data() {
    return {
      tableLoading: true,
      columns: [
  ...
  methods: {
    ...
    handleDelete(params) {
      ...
      deleteStationerOne(id).then(res => {
    ...
    _fetchData() {
      getStationerList({ ...this.queryData }).then(res => {
