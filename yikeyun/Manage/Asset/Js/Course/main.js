...
require(['../Common/init','../Common/pluploadset'], function (pluploadset) {
    require(["base", "ueditor", "ZeroClipboard", "ueditor_conf", "jqvalidate", "datetimepicker"], function (bs, UE, ZeroClipboard) {
        ...
        /*add course*/
        $courseSouceDom = $(".js-for-single")
        $("#js-course-type1").change(function(e){
            if($(this).prop("checked")) {
                $courseSouceDom.hide().find("input[type='text']").prop("disabled",true).val('')
            }
        })
        $("#js-course-type2").change(function(e){
            if($(this).prop("checked")) {
                $courseSouceDom.show().find("input[type='text']").prop("disabled",false)
            }
        })
        $addForm.validate({
            //debug:true,
            rules: {
                teacher_id: "required",
                title: 'required',
                source: {required:"#js-course-type2:checked"},
                duration: {required:"#js-course-type2:checked"}
