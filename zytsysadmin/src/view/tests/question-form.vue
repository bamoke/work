    ...
    >
      ...
      <FormItem label="类型：" prop="type">
        <Select v-model="formInfo.type" style="width:200px">
          <Option value="1">单选(必须满足某一项)</Option>
          <Option value="2">多选(满足其中之一即可)</Option>
        </Select>
      </FormItem>
      <FormItem
      ...
      >
        <Row>
          <i-col span="18">
            <Input type="textarea" :rows="2" v-model="formInfo.answer[index]" placeholder="请输入选项"/>
          </i-col>
      ...
      <FormItem label="满足条件：" prop="correct">
<!--         <RadioGroup v-model="formInfo.correct" v-if="formInfo.type !='2'">
       ...
       <FormItem label="问题释义(非必填):">
            <Editor 
            v-model="formInfo.paraphrase" 
            :cache="false" 
            :menus="editorMenus"
...
export default {
  ...
  data() {
    return {
      ...
      ruleForm: {
        ...
        correct: [{ required: true, message: "请设置满足条件" }]
      },
      formInfo: { test_id: null,type:"1", status: "1", is_lib: "0", answer: ["", ""],correct:[] },
  ...
  methods: {
    ...
    handleEditorChanged(data) {console.log(data)},
    handleAddOpt() {
